...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { LimrunInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw LimrunInvalidDataException("`$name` is not set")
            is JsonNull -> throw LimrunInvalidDataException("`$name` is null")
            else -> throw LimrunInvalidDataException("`$name` is invalid, received $this")
        }
